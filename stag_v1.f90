...
! 
!     Compile: 
!               (SGI single precision) f90 -02 stag.f90 -o stag
!               (SGI double precision) f90 -r8 -O2 stag.f90 -o stag
!
...
!.... build the u-momentum equation over the u-control volumes
      ...
      logical :: include_p
      real :: dxwr, dxer, dysr, dynr, dx, dy
      real :: sw(ny), dw(ny), cw(ny)
      real :: ss, ds, cs, sn, dn, cn, se, de, ce

!.... initialize quantities along the west boundary

      dxwr = one / (x(2)-x(1))
      do j = 2, nym
        sw(j) = pt5 * (r(j)+r(j-1)) * (y(j)-y(j-1))
        dw(j) = vis * sw(j) * dxwr
        cw(j) = pt5 * ( cx(2,j) + cx(1,j) )
      end do
...
!.... main loop - east and north sides
        ...
        do j = 2, nym
          ...
          ru(i,j) = one / ((y(j)-y(j-1))*(xc(i+1)-xc(i))) * ( &
                pt5 * ( (u(i,j)+u(i-1,j))*cw(j) - (u(i+1,j)+u(i,j))*ce ) - &
                cn * ( u(i,j+1) * fy(j) + u(i,j) * (one-fy(j)) ) + &
                cs * ( u(i,j) * fy(j-1) + u(i,j-1) * (one-fy(j-1)) ) + &
                de * ( u(i+1,j)-u(i,j) ) - dw(j) * ( u(i,j)-u(i-1,j) ) + &
                dn * ( u(i,j+1)-u(i,j) ) - ds * ( u(i,j)-u(i,j-1) ) )

          if (include_p) then
            ru(i,j) = ru(i,j) + one / ((y(j)-y(j-1))*(xc(i+1)-xc(i))) * ( &
                                sw(j) * p_old(i,j) - se * p_old(i+1,j) )
          end if
...
!.... initialize quantities along the west boundary
      ...
      dxwr = one / (xc(2)-xc(1))
      do j = 2, nymm
        sw(j) = pt5 * (r(j+1)+r(j-1)) * (yc(j+1)-yc(j))
        dw(j) = vis * sw(j) * dxwr
        cw(j) = pt5 * ( cx(1,j) + cx(1,j+1) )
      end do
...
!.... main loop - east and north sides
        ...
        do j = 2, nymm
          ...
          rv(i,j) = one / ((x(i)-x(i-1))*(yc(j+1)-yc(j))) * ( &
                pt5 * ( (v(i,j)+v(i,j-1))*cs - (v(i,j+1)+v(i,j))*cn ) - &
                ce * ( v(i+1,j) * fx(i) + v(i,j) * (one-fx(i)) ) + &
                cw(j) * ( v(i,j) * fx(i-1) + v(i-1,j) * (one-fx(i-1)) ) + &
                de * ( v(i+1,j)-v(i,j) ) - dw(j) * ( v(i,j)-v(i-1,j) ) + &
                dn * ( v(i,j+1)-v(i,j) ) - ds * ( v(i,j)-v(i,j-1) ) )

          if (include_p) then
            rv(i,j) = rv(i,j) + one / ((x(i)-x(i-1))*(yc(j+1)-yc(j))) * ( &
                                ss * p_old(i,j) - sn * p_old(i,j+1) )
          end if
...
subroutine calc_p( dtl )

!.... project the velocities to the divergence free space using a fractional
!.... step algorithm

!.... Note that p here may be either (p - p_old) or the full
!.... pressure depending on the value of delta_p

      use const
      use global
      implicit none

      real :: dtl, dtlr, rp(nx,ny)
      real :: dx, sw(ny), se, sn, ss, dw(ny), de, dn, ds
...
subroutine calc_p_full( ul, vl )

!.... This routine computes the pressure by taking the divergence of the
!.... discrete momentum equations and solving the resulting Poisson equation
...
!.... write to history file
      ...
      write(*,"('----> ',4(1pe10.3,1x),'(',i5,') full pressure')") &
           tdiv, pnorm, psum, pmax, k
      write(ipres,"(2(i5,1x),8(1pe10.3,1x))") n, k, tdiv, pnorm, psum, pmax

end subroutine calc_p_full

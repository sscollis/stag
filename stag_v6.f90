...
! Date:     12-23-99
!
! Version:  4.0 (Optimized for OpenMP)
!           6.0 Hacked immersed boundary condition
!
!============================================================================

!.... Select the correct version of library routines
...
!     axis        := Axisymmetric flag
      ...
      integer :: nx, ny, nxm, nxmm, nym, nymm, i, j, ip, jp
...
!     alfa    := SIP parameter
      ...
      real    :: time, dt, cfl, dt_max, cfl_max, tol, eps, omega, alfa
      real    :: dtau, pnorm, pmax, psum
      logical :: delta_p, first
...
!.... Boundary conditions
      ...
      integer :: bottom_bc = 0, top_bc = 0, left_bc = 0, right_bc = 0
...
end subroutine set_immersed_bc

function ramp(t,t0,t1) 

!.... Ramp up the forcing amplitude from [0,1] over the t range [t0,t1]
      ...
      if (t .le. t0) then
        ...
        ramp = 0.5 * ( 1.0 + tanh( 4.0 * ( 2.0 * (t-t0)/(t1-t0) - 1.0 ) ) )
      else
        ramp = 1.0
      end if
...
!!$      do j = 1, ny
!!$        do i = 1, nx
!!$          ud(i,j) = u_old(i,j) + fact * ru(i,j)
!!$          vd(i,j) = v_old(i,j) + fact * rv(i,j)
!!$        end do
!!$      end do
!!$
!!$!$omp parallel do private(i)
...
!!$      do j = 1, ny
!!$        do i = 1, nx
!!$          fu(i,j) = (ud(i,j) - u_old(i,j))*factr - ru(i,j)
!!$          fv(i,j) = (vd(i,j) - v_old(i,j))*factr - rv(i,j)
!!$        end do
!!$      end do
!!$      
!!$!$omp parallel do private(i)
!!$      do j = 1, ny
!!$        do i = 1, nx
!!$          ru(i,j) = ru(i,j) + fu(i,j)
!!$          rv(i,j) = rv(i,j) + fv(i,j)
!!$          ru(i,j) = ru(i,j) + ramp(time,zero,10.0) * fu(i,j)
!!$          rv(i,j) = rv(i,j) + ramp(time,zero,10.0) * fv(i,j)
!!$        end do
!!$      end do
!!$      
!!$!$omp parallel do private(i)
...
!.... build the u-momentum equation over the u-control volumes
      ...
      logical :: include_p
      real :: dxwr, dxer, dysr, dynr, volr, vol
...
!         cx(i,j) = rho * se * ( ul(i,j) - ud(i,j) )
!         cy(i,j) = rho * sn * ( vl(i,j) - vd(i,j) )
...
!$omp parallel do private(i) reduction(+:psum)
      ...
      write(*,*)'Max mass flux across virtual boundary ',maxval(abs(cy(:,jbc)))
...
!$omp parallel do private(i)
      ...
      if (pmethod .eq. 0) then
        ...
        call sip( rp, aw, ae, as, an, ap )
      else if (pmethod .eq. 5) then
        call cg( rp, aw, ae, as, an, ap )
      end if
...
module cg_mod
      real, allocatable :: a(:), rwork(:)
      integer, allocatable :: ia(:), ja(:), iwork(:)
      real :: err
      integer :: nelt, lenw, leniw, iunit=0, ierr
end module cg_mod

subroutine cg( rp, aw, ae, as, an, ap )
      ...
      nelt = 5*nxmm*nymm+2*nx+2*nymm
      lenw = 5*nx*ny+5*nx*ny
      leniw = 5*nx*ny+nx*ny+11

      if (first) then
        allocate( a(nelt), ia(nelt), ja(nelt), rwork(lenw), iwork(leniw) )
        first = .false.
      end if
...
!.... Build the SLAP triad format

      do j = 2, nym
        do i = 2, nxm
        row = i + nx*(j-1)
        
        if (aw(i,j).ne.zero) then
          nel = nel + 1
          ia(nel) = row
          ja(nel) = row-1
          a(nel)  = aw(i,j)
...
!     if (nel.ne.nelt) call error('cg$','Nel .ne. Nelt$')

!      do nel = 1, nelt
!       if (ia(nel) .eq. ja(nel)) then
!          write(*,"(2(i5,1x),1pe13.6)") ia(nel), ja(nel), a(nel)
!       end if
!      end do

      call DSICCG(nx*ny, rp, p, nelt, ia, ja, a, 0, 1, tol, kmax, k, &
                  err, ierr, iunit, rwork, lenw, iwork, leniw)

!     call DSDCG(nx*ny, rp, p, nelt, ia, ja, a, 0, 1, tol, kmax, k, &
!                err, ierr, iunit, rwork, lenw, iwork, leniw)

      if (ierr.ne.0) then
        write(*,*) ierr
!       call error('cg$','Error in Conjugate Gradient routine$')
      end if

end subroutine cg

module sip_mod
      real ,allocatable :: ue(:,:), un(:,:), lw(:,:), ls(:,:), &
                           lpr(:,:), res(:,:)
end module sip_mod

subroutine sip( rp, aw, ae, as, an, ap )

!.... SIP solver (Stronly Implicit Solver) -- currently not in parallel
...
!.... coefficients of L and U matrices (these matrices don't change)

      if (first) then
        first = .false.
        allocate( ue(nx,ny), un(nx,ny), lw(nx,ny), ls(nx,ny), &
                  lpr(nx,ny), res(nx,ny) )
        lw = zero; ls = zero; lpr = zero; un = zero; ue = zero
        do j = 2, nym
          do i = 2, nxm
            lw(i,j) = aw(i,j) / ( one + alfa * un(i-1,j) )
            ls(i,j) = as(i,j) / ( one + alfa * ue(i,j-1) )
            p1 = alfa * lw(i,j) * un(i-1,j)
            p2 = alfa * ls(i,j) * ue(i,j-1)
            lpr(i,j) = one / ( ap(i,j) + p1 + p2 - lw(i,j)*ue(i-1,j) - &
...
!$omp parallel do private(i) reduction(+: pnorm)
        do j = 2, nym
          do i = 2, nxm
            res(i,j) = rp(i,j) - an(i,j)*p(i,j+1) - as(i,j)*p(i,j-1) - &
                       ae(i,j)*p(i+1,j) - aw(i,j)*p(i-1,j) - ap(i,j)*p(i,j)
            pnorm = pnorm + res(i,j)**2
          end do
        end do
        pnorm = sqrt( pnorm / (nxmm*nymm) )
        if (k.eq.1) pnorm0 = pnorm
!       pnorm = pnorm/(pnorm0+1.0e-20)
...
#else
      write(*,*) "Need to fix LINPACK reference"
      stop
#endif
...
!.... Input run parameters and form the grid
      ...
      namelist /in/ Re, nstep, cfl_max, dt_max, tol, ictype, method, &
                    ...
                    alfa, bottom_bc, top_bc, left_bc, right_bc, ip, jp
...
!.... set default values for input variables
      ...
      pmethod=0; base='output'; nout=100; delta_p = .true.; alfa = 1.9
      first=.true.; bottom_bc=0; top_bc=0; left_bc=0; right_bc=0;
      ip=2; jp=2
...
!.... allocate mass flux variables
      ...
      allocate( cx(nx,ny), cy(nx,ny), ru(nx,ny), rv(nx,ny), &
           fu(nx,ny), fv(nx,ny), ud(nx,ny), vd(nx,ny) )
...
!$omp parallel do private(i)
      do j = 1, ny
        do i = 1, ny
          ...
          fu(i,j) = zero; fv(i,j) = zero; ud(i,j) = zero; vd(i,j) = zero
        end do
      end do
...
!.... Bottom
      ...
      select case (bottom_bc)
      case ( 0 )
        u(1:nxm,1) = zero            ! no slip
        v(1:nx,1)  = zero
      case ( 1 )
        u(1:nxm,1) = u(1:nxm,2)      ! slip
        v(1:nx,1)  = zero
      case default
        call error('bc$','Illegal bottom bc code$')
      end select
...
!.... Top
      ...
      case ( 0 )
        ...
        v(1:nx,nym) = zero
      case ( 1 )
        u(1:nxm,ny) = u(1:nxm,ny-1)  ! slip
        v(1:nx,nym) = zero
      case default
        call error('bc$','Illegal top bc code$')
      end select
...
!.... Left
      ...
      case ( 1 )
        ...
        v(1,1:nym) = v(2,1:nym) 
      case default
        call error('bc$','Illegal left bc code$')
      end select
...
!.... Right
      ...
      case ( 0 )
        ...
        v(nx,1:nym) = zero
      case ( 1 )
        u(nxm,1:ny) = zero          ! slip
        v(nx,1:nym) = v(nx-1,1:nym)
      case default
        call error('bc$','Illegal right bc code$')
      end select
